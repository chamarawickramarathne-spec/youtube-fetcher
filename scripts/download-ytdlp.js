...
async function main() {
  ...
  await download('https://github.com/yt-dlp/yt-dlp/releases/latest/download/yt-dlp.exe', ytdlpDest)

  // ffmpeg essentials build (static, Windows x64)
  ...
  if (!fs.existsSync(ffmpegDest)) {
    ...
    await download('https://www.gyan.dev/ffmpeg/builds/ffmpeg-release-essentials.zip', zipDest)

    console.log('Extracting ffmpeg.exe...')
    const { execSync } = require('child_process')
    // Use PowerShell to extract only ffmpeg.exe from the zip
    try {
      const zipName = fs.readdirSync(RESOURCES).find(f => f === 'ffmpeg.zip')
      // Find ffmpeg.exe inside the zip structure
      execSync(
        `powershell -Command "` +
...
main().catch((err) => {
  ...
  console.error('  yt-dlp.exe: https://github.com/yt-dlp/yt-dlp/releases/latest/download/yt-dlp.exe')
  console.error('  ffmpeg.exe: https://www.gyan.dev/ffmpeg/builds/ffmpeg-release-essentials.zip')
