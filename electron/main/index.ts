...
let updateState: UpdateState = { state: 'idle', version: '', percent: 0, message: '' }

function pushUpdateState(): void {
  send('update:status', updateState)
}
...
function setupAutoUpdater(): void {
  ...
  autoUpdater.on('checking-for-update', () => {
    updateState = { ...updateState, state: 'checking', message: 'Checking for updates...' }
    pushUpdateState()
  })
  autoUpdater.on('update-available', (info) => {
    updateState = { state: 'available', version: info.version || '', percent: 0, message: `Version ${info.version} is available` }
    pushUpdateState()
  })
  autoUpdater.on('update-not-available', () => {
    updateState = { ...updateState, state: 'not-available', message: 'You are up to date' }
    pushUpdateState()
  })
  autoUpdater.on('download-progress', (progress) => {
    updateState = { ...updateState, state: 'downloading', percent: Math.round(progress.percent || 0), message: `Downloading update... ${Math.round(progress.percent || 0)}%` }
    pushUpdateState()
  })
  autoUpdater.on('update-downloaded', (info) => {
    updateState = { ...updateState, state: 'downloaded', version: info.version || updateState.version, message: `Version ${info.version} downloaded` }
    pushUpdateState()
  })
  autoUpdater.on('error', (err) => {
    updateState = { ...updateState, state: 'error', message: err.message || 'Update check failed' }
    pushUpdateState()
  })
}
...
ipcMain.handle('update:check', async () => {
  if (!app.isPackaged) {
    updateState = { state: 'error', version: '', percent: 0, message: 'Updates are only available in the installed app' }
    return updateState
  }
  updateState = { ...updateState, state: 'checking', message: 'Checking for updates...' }
  pushUpdateState()
...
ipcMain.handle('update:download', async () => {
  if (updateState.state === 'available') autoUpdater.downloadUpdate()
  return true
})
